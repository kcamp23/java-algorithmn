...
let employeeDepartment = fs.readFileSync('./2_employeedepartment.txt', {encoding: 'utf8'})
...
for (let i = 0; i < employeeData2.length; i++) {
    ...
    employeeDepartmentData.employeeId[employeeData2[i].split(",")[1][3] - 1].push(employeeData2[i].split(",")[0])

    employeeDepartmentData.hireStartDate[employeeData2[i].split(",")[1][3] - 1].push(employeeData2[i].split(",")[2])

    employeeDepartmentData.hireDateStatus[employeeData2[i].split(",")[1][3] - 1].push(employeeData2[i].split(",")[3]) 
}
...
for (let i = 0; i < employeesData3.length; i++) {
    ...
    console.log(`employees data current id: ${employeesData3[i].split(",")[0]}`)
    ...
    for (let j = 0; j < employeeDepartmentData.employeeId.length; j++) {
        for (let k = 0; k < employeeDepartmentData.employeeId[j].length; k++) {
            ...
            if (employeesData3[i].split(",")[0] == employeeDepartmentData.employeeId[j][k]) {
                employeeDepartmentData.dob[j].push(employeesData3[i].split(",")[1])
                employeeDepartmentData.firstName[j].push(employeesData3[i].split(",")[2])
                employeeDepartmentData.lastName[j].push(employeesData3[i].split(",")[3])
...
console.log(employeeDepartmentData);